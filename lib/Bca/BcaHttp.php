...
class BcaHttp
{
    ...
    /**
     * Inquiry Status VA (Virtual Account) Payment
     *
     * @param string $oauth_token nilai token yang telah didapatkan setelah login
     * @param array $companyCode BCA create a unique code to identify Company who registered in BCA
     * @param string $customerNumber VA CustomerNumber (the bill number given by the Merchant
     * @param string $requestId Unique transaction identifier which generated by BCA and part or the flag payment response
     *
     * @return \Unirest\Response
     */
    public function inquiryStatusVa($oauth_token, $companyCode, $customerNumber, $requestId)
    {
        $uriSign = "GET:/va/payments?CompanyCode=$companyCode&CustomerNumber=$customerNumber&RequestID=$requestId";
        $isoTime = self::generateIsoTime();
        $authSignature = self::generateSign($uriSign, $oauth_token, $this->settings['secret_key'], $isoTime, null);
        $headers = array();
        $headers['Accept'] = 'application/json';
        ...
        $headers['X-BCA-Timestamp'] = $isoTime;
        $headers['X-BCA-Signature'] = $authSignature;
        $request_path = "va/payments?CompanyCode=$companyCode&CustomerNumber=$customerNumber&RequestID=$requestId";
        $domain = $this->ddnDomain();
        $full_url = $domain . $request_path;
